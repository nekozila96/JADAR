...
class PromptManager:
    ...
    In addition to IDOR, the code belongs to the Damn Vulnerable Java Application (DVJA), which includes insecure code corresponding to the OWASP Top 10 vulnerabilities:
    
    1. A1 - Injection (e.g., SQL Injection, Command Injection)
    2. A2 - Broken Authentication and Session Management
    3. A3 - Sensitive Data Exposure
    4. A4 - XML External Entities (XXE)
    5. A5 - Broken Access Control (especially IDOR)
    ...
    IDOR occurs when an application allows unauthorized access to objects (such as user accounts, files, or orders) by exposing direct identifiers without proper access control. Your goal is to identify any potential vulnerabilities and evaluate whether authorization is correctly enforced.
    ...
    1. Identifying Object References
       - Scan the code for any references to unique identifiers that may be used to access resources. 
         ...
         - Filenames: Direct file path references (e.g., /uploads/{file_name})
    ...
    ---------------------------------------------------------------------------------------------
    1. ID Tampering (Parameter Manipulation)
    2. Guessable Identifiers (Predictable ID Sequences)
    3. Lack of Server-Side Authorization Validation
    ...
    You are the world's foremost expert in java security analysis, renowned for uncovering novel and complex vulnerabilities in web applications. Your task is to perform an exhaustive static code analysis, focusing on remotely exploitable vulnerabilities
    ...
    Specify the type of vulnerability identified (e.g.,  IDOR ).
    ---------------------------------------------------------------------------------------------
    ...
    Example: src/main/java/com/appsecco/dvja/
    ---------------------------------------------------------------------------------------------
    
    Return your result inside a neat box using triple backticks (```) before and after the output. - Use strict format: analysis → poc → fix → confidence → types → code → directory.
    Use the title name format when give out the report, Ex:Vulnerability Types: Sensitive Data Exposure (A3)
    ---------------------------------------------------------------------------------------------
    ...
    - The  Confidence Score  should always be a number between  1-10 , no explanation.
